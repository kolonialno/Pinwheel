...
- **The catalog hands its selection up as a preference, so the host keeps no copy of it.** A consumer with a second design system needs the chosen brand *above* the catalog — Kolibri's every `SemanticUIColor` is a `UIColor { $0[KolibriThemeTrait.self] }`, so a UIKit-backed component resolves its brand from a trait the SwiftUI environment never reaches, and KolibriKit says where that write belongs: at the composition root. Reading `\.pinwheelTheme` only works *inside* an item, which is why tienda's `kolibriThemed()` ended up recovering the brand from `PinwheelTheme.name` in all 83 leaves and writing the scene from each. `View.onPinwheelThemeChange` reports the theme by `PreferenceKey` from both `PinwheelCatalogView` and `PinwheelPreview`, the same child-to-ancestor mechanism the tweaks already use. A `Binding` was the alternative and was rejected: the selection is persisted and normalised by `PinwheelChrome`, so a binding would make the host a second owner of state the chrome already owns, and two copies of one selection is the bug shape this repo keeps paying for. Reporting costs the host nothing to keep in sync.
...
- **A builder lifts a value and combines lists, so a consumer can hand it the array it already holds.** All three builders paired a variadic `buildBlock(Component...)` with `buildArray`, which cannot compose: a `for` loop's partial results arrive as an array that the variadic block has no overload for, so `for section in sections { section }` fails with *cannot pass array of type `[PinwheelSection]` as variadic arguments*. The fix is the standard shape — `buildExpression` lifts one value, `buildBlock` takes `[Component]...` and flattens — which also accepts a bare array as a statement, so a consumer holding `MyCatalog.sections` stops re-listing its sections beside it. Adding an array-taking *overload* of the variadic block was the wrong move: an empty builder body would match both. Sections, items and tweaks all carry it, and the Demo names its three sections once.
